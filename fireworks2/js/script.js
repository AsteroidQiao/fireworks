...
function randomShellName() {
    return Math.random() < 0.5 ? '菊花簇' : shellNames[(Math.random() * (shellNames.length - 1) + 1) | 0];
}
...
const fastShellBlacklist = ['炸裂', '四叶草', '菊花簇'];

function randomFastShell() {
    const isRandom = shellNameSelector() === '随机种类';
...
const shellTypes = {
    ...
    '炸裂': crackleShell,
    '四叶草': crossetteShell,
    '菊花簇': crysanthemumShell,
    '碎花': fallingLeavesShell,
    '散花(二级爆炸)': floralShell,
    '繁星': ghostShell,
    '马尾': horsetailShell,
    '手掌': palmShell,
...
document.addEventListener("DOMContentLoaded", () => {
    window.wallpaperPropertyListener = {
        applyUserProperties: (properties) => {
            if (properties.sound) {
                params.sound = properties.sound.value;
                toggleSound(params.sound);
            }
            if (properties.background_image) {
                params.bgImage = properties.background_image.value;
                let imageEl = document.getElementById('bgimage');
                if (params.bgImage)
                    imageEl.style.backgroundImage = 'url("file:///' + properties.background_image.value + '")';
                else
                    imageEl.style.backgroundImage = '';
            }
            if (properties.fit_screen) {
                params.fitScreen = properties.fit_screen.value;
                var imageEl = document.getElementById('bgimage');
                params.fitScreen ? imageEl.style.backgroundSize = "cover" : imageEl.style.backgroundSize = "";
            }
            if (properties.invert) {
                params.invert = properties.invert.value;
                applyFilters();
            }
            if (properties.grayscale) {
                params.grayscale = properties.grayscale.value;
                applyFilters();
            }
            if (properties.blur) {
                params.blur = properties.blur.value;
                applyFilters();
            }
            if (properties.contrast) {
                params.contrast = properties.contrast.value;
                applyFilters();
            }
        }
    };
...
function applyFilters() {
    var imageEl = document.getElementById('bgimage');
    imageEl.style.filter = `grayscale(${params.grayscale}%) 
							blur(${params.blur}px)
							contrast(${params.contrast}%)
							${params.invert ? "invert(1)" : ""}`;
}